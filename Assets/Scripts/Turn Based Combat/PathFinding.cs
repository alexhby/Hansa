...
public class PathFinding : MonoBehaviour
{
    ...
    public void showAvailableMoves(Vector3 startPos, int agility)
    {
        ...
        while (openSet.Count > 0)
        {
            ...
            Vector2 wp = c.GetPointInCellFromTileIndex(currentTile.TileIndex);
            Vector2 currentTilePos = c.convertIndexToWorldPos((int)wp.x, (int)wp.y);
    ...
    public int FindPath(Vector3 startPos, Vector3 targetPos)
    {
        ...
        Tile targetTile = c.GetTileFromPointInCell((int)t2.x, (int)t2.y);
    ...
    void RetracePath(Tile start, Tile end)
    {
        ...
        {
            path.Add(current);
            Vector2 wp = c.GetPointInCellFromTileIndex(current.TileIndex);
            Vector2 currentTilePos = c.convertIndexToWorldPos((int)wp.x,(int)wp.y);
